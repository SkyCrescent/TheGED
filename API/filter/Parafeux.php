...
if (isset($_GET['recepteur'] ,$_GET['Type_transmission'] , $_GET['expediteur'],$_GET['parafeux'] )) {
...
//    die();
    ...
    $query->bind_param("ssss", $recepteur ,$Type_transmission,$expediteur,$parafeux);
    if ($query->execute()) {
...
//        $answer['num_rows'] = $numRows;
...
} else if (isset($_GET['recepteur'] ,$_GET['Type_transmission'] , $_GET['objet'] ,$_GET['parafeux']  )) {
    $recepteur = $_GET['recepteur'];
    $Type_transmission = $_GET['Type_transmission'];
    $objet = $_GET['objet'];
    $parafeux = $_GET['parafeux'];
   // $query = $con->prepare("SELECT id,objet, expediteur,date,parafeux,Niveau,transmission_interne,note FROM courier WHERE recepteur = ? and Type_transmission = ? and objet LIKE CONCAT( ?, '%') ORDER BY 1 DESC");
    ...
    $query->bind_param("ssss", $recepteur ,$Type_transmission,$objet,$parafeux);
    if ($query->execute()) {
        $query->store_result(); // Stocke les résultats dans le client
        $numRows = $query->num_rows;
        $query->bind_result( $id, $objet, $expediteur,$date,$parafeux,$Niveau ); // Change $nom to $resultNom
...
//        $answer['num_rows'] = $numRows;
   ...
   // $query = $con->prepare("SELECT id,objet, expediteur,date,parafeux,Niveau,transmission_interne,note FROM courier WHERE recepteur = ? and Type_transmission = ? and expediteur = ?  and objet = ? ORDER BY 1 DESC");
    $query = $con->prepare("SELECT id,objet, expediteur,date,parafeux,Niveau FROM courier WHERE recepteur = ? and Type_transmission = ? and parafeux = ? and expediteur = ?  and objet LIKE CONCAT( ?, '%') ORDER BY 1 DESC ");
