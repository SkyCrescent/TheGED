...
if (isset($_GET['expediteur'] ,$_GET['Type_transmission'] )) {
  ...
  $Type_transmission = $_GET['Type_transmission'];
    ...
    $query = $con->prepare("SELECT id,objet, recepteur,date,Niveau FROM courier WHERE expediteur = ? and Type_transmission = ?");
    $query->bind_param("ss", $expediteur ,$Type_transmission);
    if ($query->execute()) {
        $query->store_result(); // Stocke les résultats dans le client
        $numRows = $query->num_rows;
        $query->bind_result( $id, $objet, $recepteur,$date,$Niveau ); // Change $nom to $resultNom
