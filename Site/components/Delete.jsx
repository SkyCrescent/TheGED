...
export default function Delete({updateValueNotification,handleClickButton8, updateValueNotifications,SetDelete,valueDelete}) {
   const baseUrl = process.env.NEXT_PUBLIC_API_URL;
   const [loading , SetLoading ] = useState(false)
   const pathname = usePathname();

   useEffect(()=>{
      console.log('lid du courrrier',valueDelete)
   ...
   const deleteData  = async () => {
      ...
      try {
         ...
         const response = await axios.post(`${baseUrl}/doc_structure/delete_doc_Structure.php?id=${valueDelete}`,  {
            headers: {
               'Content-Type': 'multipart/form-data',
            },
         });

         const newValue =  'delete'
            /* Nouvelle valeur */;
   ...
   const deleteData2  = async () => {
      ...
      try {
         ...
         const response = await axios.post(`${baseUrl}/doc/delete_doc2.php?id=${valueDelete}`,  {
            headers: {
               'Content-Type': 'multipart/form-data',
            },
         });
            ...
            <div className="w-full flex justify-center my-52 md:my-96 lg:my-60">
               <div
                  ...
                  <div
                     ...
                     <button
                        className="w-[50%] bg-red-600 flex items-center  justify-center gap-3 mx-6 mt-1 hover:bg-red-900 text-white transition duration-300 transform hover:scale-105  rounded-md text-xs font-semibold p-1.5"
                       // onClick={pathname.includes('director') ? handleClickButton1 : handleClickButton2}
                        onClick={pathname.includes('director') ? deleteData2 : deleteData}
