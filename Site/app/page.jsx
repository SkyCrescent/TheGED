...
export default function Home() {
   ...
   const getData = async () => {
      try {
         const response = await axios.get(`${baseUrl}/type_structure/get_byId2.php`);

         if (response.data && response.data.recu && response.data.recu.length > 0) {

            console.log( response.data.recu[0].Description )
            Setdescription(response.data.recu[0].Description)

            if ((response.data.recu[0].dar !== formattedDate)  && (response.data.recu[0].Description !== '0' ) ){
               theday = (response.data.recu[0].Description) -1
               console.log(theday)
               console.log(formattedDate)
               const response2 = await axios.get(`${baseUrl}/type_structure/updateCommentaire.php?description=${theday}&dar=${formattedDate}`);
               setOption(false)
               console.log(response2)
            }else if ( response.data.recu[0].Description === '0' ) {
               setOption(true)
               console.log("c fait")
            }
            ...
            <div
               ...
               <div className="h-[100%] w-[55%]  flex items-center mt-8 rounded ">
                  ...
                  <div className="relative h-[60%] mx-3 items-center  w-[100%]  ">
                     ...
                     <h1 className="text-sm py-2 mx-2 cursor-default">Insérez les informations de votre Compte Utilisateur afin d'accéder à votre Workspace.</h1>
                     <div className="-py-3 -mx-1  ">
                        ...
                        <h1
                           ...
                           {NotValue ? "Saisissez vos informations" : NotResult ? "Compte introuvable, réessayez" : CompteLocked ? "Ce compte a ete bloqué ,contacter votre Administrateur afin de le débloquer" : ''}
                        </h1>
                  ...
                  <div className="relative w-[80%] h-[20%] mx-5 space-y-4  ">
                     <div className="w-full  flex items-center justify-center">
                        <button
                           className="w-full lg:w-fit bg-indigo-600 hover:bg-indigo-950 text-white transition duration-300 transform hover:scale-105 px-28 py-4 -mx-6 rounded-md font-semibold"
                           onClick={() => {
                                option ?  router.push(`expired`) :   handleSummit()
                           }}>Connexion
                        </button>
