...
export default function page() {
   ...
   const getData2 = async () => {
      try {
         ...
         const response = await axios.get(`${baseUrl}/agent/get_allusername.php`);
...
const handleChange3 = async (e) => {
   ...
   if (values.service  && e.target.value === "Directeur Général") {
      try {
            const response = await axios.get(`${baseUrl}/verification/verification.php?id_structure=${values.service}&poste_agent=Directeur Général`);
...
// Fonction pour vérifier si l'username est libre
   const getData4 = async () => {
      try {
         const response = await axios.get(`${baseUrl}/verification/verificationUsername.php?username=${values.username}`);
                              ...
                              : reponse === 'oui' || etat === 12 ? 'Un directeur est déjà enregistré dans la structure sélectionné'
                                 : reponse2 === 'oui' ? 'Cet username est déjà utilisé par un autre utilisateur'
                                    : etat === 20 ? 'Saisissez vos Informations'
                                       ...
                                       : etat === 2 ? 'Saisissez votre Prénom'
                                          : etat === 3 ? 'Saisissez votre Adresse'
                                             : etat === 4 ? 'Saisissez votre Numéro de téléphone '
                                                : etat === 5 ? '  Sélectionnez votre situation Matrimoniale'
                                                   : etat === 6 ? 'Sélectionnez votre Structure'
                                                      : etat === 7 ? 'Saisissez votre Poste'
                                                         : etat === 8 ? 'Saisissez votre Username'
                                                            ...
                                                            : etat === 10 ? 'Saisissez votre password'
                                                               : etat === 11 ? "Sélectionez votre photo d'utilisateur"
